...
describe('remove tab', function() {
  ...
  iit('should remove title panes when elements are destroyed', inject(function($controller, $compile, $rootScope) {
    var scope = $rootScope;
    var elm = $compile("<tabs><pane ng-repeat='i in list' title='{{i}}'>{{i}}</pane></tabs>")(scope);
    scope.$apply();
    expect(elm.find('.tab-pane').length).toBe(0);
    scope.$apply('list = [1,2,3]');
    expect(elm.find('.tab-pane').length).toBe(3);
    expect(elm.find('.nav-tabs li').length).toBe(3);
    scope.$apply('list = [1]');
    waits(100);
    dump(elm.find('.tab-pane'));
    dump(elm.find('.nav-tabs li'));
    expect(elm.find('.tab-pane').length).toBe(1);
    expect(elm.find('.nav-tabs li').length).toBe(1);
  }));
